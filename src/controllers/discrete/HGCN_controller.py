...
class HGCNMAC(BasicMAC):
    ...
    def __init__(self, scheme, groups, args):
        ...
        self.hgcn_input_shape = 2

        # 初始化 HGCN_EMBEDDING
        self.hgcn_embedding = HGCN_EMBEDDING(
            in_feature_dim=self.hgcn_input_shape,        # 输入特征维度
        ...
        self.hgcn_hidden = None  # 新增：用于存储 HGCN_EMBEDDING 的隐藏状态
    ...
    def forward(self, ep_batch, t, test_mode=False):
        ...
        # 构造智能体输入
        agent_inputs = self._build_inputs(ep_batch, t)  #[batch_size*n_agents,67]c?

        # 通过 HGCN_EMBEDDING 生成超图嵌入并更新隐藏状态
        #input_shape=self.in_feature_dim*self.n_agents,
            # hidden_dim=self.n_hyper_edges*self.n_agents*2,
            # hyper_graph_mat=self.n_hyper_edges*self.n_agents

        # time_hgcn_embedding_before = time.time()

        if self.args.hg_generate_type == "certain": 
            hgcn_embedding, self.hgcn_hidden= self.hgcn_embedding(
            agent_inputs.view(batch_size, self.n_agents, -1)[:,:,0:self.hgcn_input_shape],  # 传入正确形状的输入, only [0:2] in obervation is used
            self.hgcn_hidden  # 传入当前隐藏状态
        )
        elif self.args.hg_generate_type == "bernoulli":
            ...
            #error
            raise ValueError(f"unsupported hg_generate_type: {self.args.hg_generate_type}")


        
        # time_hgcn_embedding_after = time.time()
        # print("time_hgcn_embedding:", (time_hgcn_embedding_after - time_hgcn_embedding_before)/1000)
        #
        # 将智能体输入和 HGCN 嵌入拼接
        if self.args.concate_gcn:
        ...
        # 通过 agent 网络
        # time_agent_before = time.time()
        agent_outs, self.hidden_states = self.agent(agent_inputs, self.hidden_states)
        # time_agent_after = time.time()
        # print("time_agent:", (time_agent_after - time_agent_before)/1000)
