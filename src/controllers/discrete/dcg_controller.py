...
class DeepCoordinationGraphMAC(BasicMAC):
    ...
    def __init__(self, scheme, groups, args):
        ...
        if self.use_action_repr:
            ...
            # th.cat([input_i, input_j], dim=-1) -> [18,18,40]
            #  .view(self.n_actions * self.n_actions, -1) -> [324,40]
            #   .t().unsqueeze(0) -> [40, 324] -> [1, 40, 324]
            # p_action_repr  [1,40,324]
    ...
    def payoffs(self, hidden_states):
        ...
        if self.payoff_decomposition:
            ...
            # output [2, 1, 45, 324] -> [2,1,45,18*18]
            ...
            # dim = list(output.shape[:-1]) # (2,1,45)
            # output = output.view([*dim*self.payoff_rank, n,n]) # self.payoff_rank only can be 1
            # output = output.reshape(-1,n,n)
            # transpose_output= output.transpose(2,1)
            # output = th.bmm(output, transpose_output)
            # output = output.view([*dim, n,n])
        else:
            # Without decomposition, the payoff_fun output must only be reshaped
            output = output.view(*(list(output.shape[:-1]) + [n, n]))
            # torch.Size([2, 1, 45, 18, 18])

        # The output of the backward messages must be transposed
        output_transposed = output[1].transpose(dim0=-2, dim1=-1)

        # Concatenate output[0], output_transposed, and the remaining output tensors along dimension 0
        output = th.cat([output[0].unsqueeze(0), output_transposed.unsqueeze(0), output[2:]], dim=0)
