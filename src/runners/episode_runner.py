...
class EpisodeRunner:
    ...
    def run(self, test_mode=False):
        ...
        episode_env_info = {} #env_info 中的奖励信息累加起来
        ...
        while not terminated:
            ...
            # time_env_step_before=time.time()
            _, reward, terminated, truncated, env_info = self.env.step(actions[0])
            # time_env_step_after=time.time()
            # print(f"env one step cost time :{(time_env_step_after-time_env_step_before)/1000}")
            terminated = terminated or truncated
            if test_mode and self.args.render:
                self.env.render()
            episode_return += reward
            for k, v in env_info.items():
                    if k not in episode_env_info:
                        episode_env_info[k] = v
                    else:
                        episode_env_info[k] += v
        ...
        cur_stats.update(
            {
                k: cur_stats.get(k, 0) + episode_env_info.get(k, 0)
                for k in set(cur_stats) | set(episode_env_info)
            }
        )
