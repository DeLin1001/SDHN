...
# 注意：state_* 方法是静态的，因为否则 getattr 会记住 self

def state_xyz_vxyz_R_omega_goal(self):
    if self.use_numba:
    ...
    # 修改处：在返回的观测中添加 self.goal
    return np.concatenate([pos, vel, rot.flatten(), omega, self.goal])
